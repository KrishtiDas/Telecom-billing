...
printf("\n 3 : for modifying records.\n 4 : for payment purpose");
printf("\n 5 : for record searching.");
printf("\n 6 : for exit\n");
scanf("%d",&choice);
switch(choice)
{
...
printf("The record 1 is successfully added ");
...
exit(0);
fflush(stdin);
printf("Enter the phone number to search in our database : ");
scanf("%s", phonenumber);
while(fread(&s,sizeof(s),1,f)==1)
{
...
{
printf("The requested phone number is not found in our database ....");
}
}
fclose(f);
}
...
if((f=fopen("file.ojs","rb+"))==NULL)
exit(0);
printf("Enter the phone number of the subscriber for payment : ");
scanf("%s[^\n]",phonenumber);
fflush(stdin);
while(fread(&s,sizeof(s),1,f)==1)
{
